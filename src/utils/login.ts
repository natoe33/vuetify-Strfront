...
export class LoginUtil {
  static getHexFromPrivateOrPubKey(key: string): string {
    ...
    const insecureMsg =
      "Can't login with private key on an insecure connection, please use a Nostr key manager (NIP-07) extension instead";
  ...
  static hexToBech32(idType: string, hex?: string) {
    ...
    try {
      if (
        idType === NostrPrefix.Note ||
        idType === NostrPrefix.PrivateKey ||
        idType === NostrPrefix.PublicKey
      ) {
  ...
  static encodeTLV(
    ...
    const buf =
      prefix === NostrPrefix.Address ? enc.encode(id) : utils.hexToBytes(id);
    ...
    const tl3 = kind
      ? [3, 4, ...new Uint8Array(new Uint32Array([kind]).buffer).reverse()]
      : [];
